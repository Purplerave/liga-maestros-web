...
async function refreshData(options = {}) {
    ...
    try {
        // First paint optimization for mobile / cold load: fetch lightweight ?first=1
        // to render matches and ticket immediately before the heavy full payload resolves.
        if (!options.auto && !state.data) {
            try {
                const firstRes = await fetchLigaDataWithRetry(`/api/liga/data?j=${encodeURIComponent(state.jornada)}&first=1`);
                if (firstRes.ok) {
                    const firstData = await firstRes.json();
                    state.data = firstData;
                    state.jornada = String(firstData.jornada || state.jornada);
                    hydrateJornadaNav();
                ...
                console.warn("First paint fetch failed, proceeding with full fetch", firstError);
            }
        }
        ...
        state.data = Object.assign({}, state.data || {}, fullData);
