...
from ..services.quiz import (
    get_quiz_preguntas,
    get_quiz_ranking_jornada,
    get_quiz_ranking_mensual,
    get_quiz_ranking_temporada,
    get_user_quiz_stats,
    submit_quiz_respuestas,
)
...
def quiz_submit():
    ...
    if not attempt:
        return jsonify(
            {"status": "error", "code": "QUIZ_ATTEMPT_MISSING", "message": "Debes iniciar el reto antes de enviarlo."}
        ), 400
    try:
        started_at = datetime.fromisoformat(str(attempt.get("started_at")))
        if started_at.tzinfo is None:
            started_at = started_at.replace(tzinfo=UTC)
        tiempo_ms = max(0, int((datetime.now(UTC) - started_at).total_seconds() * 1000))
    except (TypeError, ValueError):
        return jsonify(
            {"status": "error", "code": "QUIZ_ATTEMPT_INVALID", "message": "Debes iniciar el reto antes de enviarlo."}
        ), 400
        ...
        return jsonify(
            {
                ...
                "tipo": "jornada",
                "jornada": int(jornada),
                "ranking": publicize_identifiers(ranking, (session.get("user") or {}).get("id")),
            }
        )
