...
def submit_quiz_respuestas(jornada, user_id, nombre, respuestas, tiempo_total_ms=0, expected_question_ids=None):
    ...
    try:
        ...
        existing = conn.execute(
            "SELECT id FROM quiz_participaciones WHERE user_id = ? AND jornada = ?", (user_id, jornada)
        ).fetchone()
        if existing:
            return {"error": "Ya has participado en el Reto 10 de esta jornada."}

        preguntas = conn.execute(
            "SELECT id, respuesta_correcta, dificultad FROM quiz_preguntas WHERE jornada = ? AND activa = 1", (jornada,)
        ).fetchall()
...
def get_quiz_ranking_temporada():
    ...
    try:
        ...
        for idx, row in enumerate(rows, 1):
            ...
            entry["posicion"] = idx
            entry["hit_rate"] = (
                round((entry["aciertos_totales"] / entry["total_preguntas"]) * 100, 1)
                if entry["total_preguntas"]
                else 0
            )
            result.append(entry)
        return result
